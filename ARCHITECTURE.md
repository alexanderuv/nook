...
- With documents reachable only through the core service, the agent has **no adapter
...
| Layer            | Choice                                     | Notes |
| ---------------- | ------------------------------------------ | ----- |
| Modules          | core service + `:web-app` + `:mcp-server` (+ shared contract) | The **core service** owns the stores and the single write path and exposes an internal RPC API, speaking JSON-RPC 2.0 ([ADR-2](./architecture/adrs/adr-2.md)); the web app and MCP server are **thin adapter apps** that call it and hold no store access. A shared contract library carries the DTOs and the one client both adapters use to reach the internal RPC API, so that neither can read a reply differently. |
...
this "no authentication" assumption is revisited — the entry point is gated at the edge
...
| Wire shape | **JSON-RPC 2.0** ([ADR-2](./architecture/adrs/adr-2.md)), one shape on both adapters: the core serves it on its internal RPC API and the web app serves that same shape outward, adding the access gate, HTTPS, and the UI rather than a translation ([docs/01](./docs/01-interface-contracts.md)). Nook has no wire shape of its own; the contract library defines only the operations' parameters and entities. | *A web API with a shape of its own* — first chosen, then reversed: mirroring the catalog with the project in a path segment and the outcome in an HTTP status number meant two designs over one operation set, two places to change a field, and a second contract for the UI; it also put two failure codes on `409` and made a route that matched nothing indistinguishable, by number, from an item that is not there. *A shared envelope designed here* — chosen next, then reversed (ADR-2): an `outcome` field naming one of three endings, with four string codes, is JSON-RPC 2.0 under different names, and `:mcp-server` already spoke JSON-RPC because MCP is defined on it. *REST with RFC 9457 Problem Details* — the serious alternative to the standard finally adopted; it keeps HTTP-native semantics and cacheable reads, but reinstates two designs over one action-heavy operation set. *MCP proxying the web API to collapse the two* — deletes nothing, since the web app still calls the core, and adds a hop plus a second client. |
