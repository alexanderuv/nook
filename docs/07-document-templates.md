...
Nook ships templates for **ten authored artifact types**, spanning the workflow
stages *frame → discover → define → decide → build → learn*. Each answers a
distinct question. Kinds are level-agnostic in the schema ([02](./02-document-layer.md));
...
| Discover | Research report | `research` | epic or leaf | *What did we learn?* — spike findings, user research, competitive analysis, experiment results |
...
| Decide | ADR | `adr` | epic | *What did we decide, and why?* — context, decision, consequences |
| Decide | Design doc | `design_doc` | epic | *How will it work technically?* — architecture, data model, interfaces, risks |
| Build | Implementation plan | `plan` | leaf | *How do I build this one thing?* — analysis, background, approach, caveats, test plan |
...
research report about an A/B test; a PR/FAQ is a frame-stage doc in Amazon's
...
- **Experiment plan/report, competitive analysis, user-research writeup, spike
  report, discovery audit** — research variants. **Eval spec** — test-plan
  variant.
...
- **Charter, OKRs, business case/BRD, architecture overview, glossary /
  domain-model doc** — project-level documents, deferred with [02]'s `/docs/`
  area. (The glossary is a likely early addition there — shared vocabulary is
  disproportionately valuable to agents.)
...
- Adds document kinds `prd` (7), `adr` (8), `research` (9), `test_plan` (10),
  `retro` (11), `spec` (12) to [02](./02-document-layer.md) and the structure
  schema (`db/changelog`).
